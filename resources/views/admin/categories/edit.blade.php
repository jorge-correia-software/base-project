        ...
        <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <div class="bg-gradient-dark shadow-dark border-radius-lg pt-2-5 pb-2-5">
                    <h6 class="text-white text-capitalize ps-3 mb-0">Edit Category</h6>
                </div>
            </div>
            <div class="card-body pb-2">
                <form action="{{ route('admin.categories.update', $category) }}" method="POST">
                    ...
                    <fieldset class="form-fieldset mb-4">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="input-group input-group-outline mb-2 {{ old('slug', $category->slug) ? 'is-filled' : '' }}">
                                    <label class="form-label">Slug</label>
                                    <input type="text" id="slug" name="slug" class="form-control" value="{{ old('slug', $category->slug) }}" readonly style="pointer-events: none; cursor: not-allowed; background-color: #f8f9fa;">
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="input-group input-group-outline mb-3 {{ old('description', $category->description) ? 'is-filled' : '' }}">
                    ...
                    <div class="mb-3">
                        <p class="text-sm mb-0">Posts in this category: <strong>{{ $category->posts_count ?? 0 }}</strong></p>
                        <p class="text-sm mb-0">Created: <strong>{{ $category->created_at->format('d/m/Y') }}</strong></p>
                        <p class="text-sm mb-0">Updated: <strong>{{ $category->updated_at->format('d/m/Y') }}</strong></p>
                    </div>

                    <div class="d-flex gap-2 justify-content-end mt-3">
                        <a href="{{ route('admin.categories.index') }}" class="btn btn-secondary btn-sm">Cancel</a>
                        <button type="submit" class="btn bg-gradient-primary btn-sm">Update Category</button>
                    </div>
                </form>
            </div>
        </div>
