...
<x-page-header
    icon="delete"
    title="Deleted Pages"
    subtitle="View and manage deleted pages"
/>
...
<x-datatable
    ...
    emptyMessage="No deleted pages found"
    emptySubtitle="All deleted pages will appear here"
    emptyIcon="delete"
    :paginator="$pages"
    resourceType="page"
...
>
    ...
    <tr data-page-id="{{ $page->id }}">
        ...
        <td class="align-middle text-center">
            <span class="text-secondary text-xs font-weight-bold">{{ $page->deleted_at->format('d/m/Y') }}</span>
        </td>
    </tr>
    @endforeach
...
function restoreBulkSelected() {
    ...
    if (selectedIds.length === 0) {
        showToast('Please select at least one page to restore.', 'warning');
        return;
    }
    ...
    const message = count === 1
        ...
        : `Are you sure you want to restore ${count} pages?`;

    if (!confirm(message)) {
        return;
    }

    fetch('{{ route("admin.pages.bulk-restore") }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || 'Failed to restore pages', 'error');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showToast('An error occurred while restoring pages', 'error');
    });
}
...
function permanentlyDeleteBulkSelected() {
    ...
    if (selectedIds.length === 0) {
        showToast('Please select at least one page to permanently delete.', 'warning');
        return;
    }
    ...
    fetch('{{ route("admin.pages.bulk-permanent-delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || 'Failed to permanently delete pages', 'error');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showToast('An error occurred while permanently deleting pages', 'error');
    });
}
</script>
@endsection
