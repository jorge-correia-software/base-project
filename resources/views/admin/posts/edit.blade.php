...
<x-page-header
    icon="article"
    title="{{ $post->title }}"
    subtitle="Update post information and settings"
    ...
    <div class="col-12 col-xl-8 mx-auto">
        <form action="{{ route('admin.posts.update', $post) }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    <x-admin.card title="Edit Post" position="first">
                            ...
                            <div class="mb-3">
                                <select name="featured_image_id" id="featured_image_id" class="form-control @error('featured_image_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($media as $image)
                                        <option value="{{ $image->id }}" {{ old('featured_image_id', $post->featured_image_id) == $image->id ? 'selected' : '' }}>
                                            {{ $image->file_name }}
                                        </option>
                                    @endforeach
                                </select>
